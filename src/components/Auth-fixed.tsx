      ...
      <div className="w-full sm:w-80 md:w-96 lg:w-96 relative">
        ...
        <div className="text-center mb-6 sm:mb-8 pt-8 sm:pt-10">
          <div className="flex items-center justify-center mb-3 sm:mb-4">
            <Logo className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-greenyellow" iconClassName="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16" />
          </div>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-greenyellow mb-2 sm:mb-3 font-mono tracking-wider">BLACKIRIS</h1>
          <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} text-xs sm:text-sm md:text-base`}>{t('joinCommunity')}</p>
        </div>

        <div className={`${theme === 'dark' ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-black border-purple-600/30' : 'bg-gradient-to-br from-white to-gray-50 border-purple-200'} rounded-3xl p-6 sm:p-8 border-2 shadow-2xl min-h-[500px] sm:min-h-[600px] flex flex-col backdrop-blur-sm`}>
          <div className="flex gap-2 mb-6 sm:mb-8">
            <button
              type="button"
              onClick={() => setIsSignUp(false)}
              className={`flex-1 py-3 sm:py-4 px-4 rounded-lg font-bold text-sm sm:text-base transition-all ${
                !isSignUp
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 flex-1 flex flex-col">
            ...
            {!isCaptchaConfigured() && (
              <div className={`p-4 sm:p-5 rounded-2xl border-2 text-xs sm:text-sm flex items-start gap-3 ${
                theme === 'dark'
                  ? 'bg-blue-900/20 border-blue-500/40 backdrop-blur-sm'
                  : 'bg-blue-50 border-blue-300 backdrop-blur-sm'
              }`}>
                <Info className={`w-5 h-5 flex-shrink-0 mt-0.5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`} />
                <div>
                  <p className={`font-bold mb-1 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>CAPTCHA Not Configured</p>
                  <p className={`text-xs ${theme === 'dark' ? 'text-blue-300/80' : 'text-blue-700'}`}>For DDoS protection, add hCaptcha Site Key to .env as VITE_HCAPTCHA_SITE_KEY</p>
                </div>
              </div>
            )}

            {rateLimitWarning && (
              <div className={`p-4 sm:p-5 rounded-2xl border-2 text-xs sm:text-sm flex items-center gap-3 font-medium ${
                theme === 'dark'
                  ? 'bg-yellow-900/20 border-yellow-500/40 text-yellow-300 backdrop-blur-sm'
                  : 'bg-yellow-50 border-yellow-300 text-yellow-800 backdrop-blur-sm'
              }`}>
            ...
            {error && (
              <div className={`p-4 sm:p-5 rounded-2xl border-2 text-xs sm:text-sm font-medium ${
                theme === 'dark'
                  ? 'bg-red-900/20 border-red-500/40 text-red-300 backdrop-blur-sm'
                  : 'bg-red-50 border-red-300 text-red-800 backdrop-blur-sm'
              }`}>
                {error}
              </div>
            )}
